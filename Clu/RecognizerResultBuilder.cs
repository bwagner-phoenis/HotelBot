...
internal static class RecognizerResultBuilder
{
    ...
    /// 
    /// </summary>
    private static void UpdateRecognizerResultFromConversations(JsonElement conversationPrediction,
    ...
    private static IDictionary<string, IntentScore> GetIntents(JsonElement prediction)
    {
        ...
        {
            result.Add(intent.GetProperty("category").GetString(),
    ...
    private static IDictionary<string, IntentScore> GetIntents(JObject luisResult)
    {
        ...
        if (intents != null)
        {
            ...
            {
                result.Add(NormalizeIntent(intent.Key),
                    new IntentScore { Score = intent.Value["score"]?.Value<double>() ?? 0.0 });
            }
        }
    ...
    private static JToken MapProperties(JToken source, bool inInstance)
    {
        ...
        if (source is JObject obj)
        {
            ...
            if (!inInstance && obj.TryGetValue("type", out var type) && type.Type == JTokenType.String &&
                DateSubtypes.Contains(type.Value<string>()))
            {
            ...
            else
            {
                ...
                foreach (var property in obj.Properties())
                {
                    ...
                    else if (inInstance)
                    {
                        ...
                        {
                            nobj.Add("endIndex",
                                property.Value.Value<int>() + property.Parent["startIndex"].Value<int>());
                        }
